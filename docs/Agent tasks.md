...
### TASK-I3: Stripe Payments On Checkout And Business Registration Round-Trip Verification

- **Phase:** `Phase I - Public Checkout Backend Wiring`
- **Owner:** `Both`
- **Status:** `done`
- **Implementation side:** `Both`
- **Actor(s):** `Guest`, `Reader`, `Admin`, `Business`
- **Route(s) or endpoint(s):** `POST /api/v1/subscriptions/checkout`; `POST /api/v1/subscriptions/checkout/<session_id>/confirm`; `POST /api/v1/auth/register`; `GET /api/v1/admin/companies`; `GET /api/v1/admin/companies/<id>`; `POST /api/v1/admin/companies/<id>/approve-license`; `GET /api/v1/business/locations`
- **Files touched:** `backend/requirements.txt` (stripe==15.6.1), `backend/services/payments.py`, `backend/models/checkout_session.py`, `backend/migrations/versions/b5c2d1f0a3e8_checkout_client_secret.py`, `backend/routes/checkout.py`, `backend/routes/stripe_webhooks.py` (new), `backend/app.py` (webhook blueprint + `/api/v1/webhooks/stripe`), `backend/.env.example` (`STRIPE_ENDPOINT_SECRET`), `frontend/src/lib/checkout-store.js`, `frontend/src/pages/SubscribeCheckout.jsx`, `frontend/src/pages/SubscribeSuccess.jsx`, `frontend/src/api/backendClient.js` (place auth, from earlier fix), `frontend/src/pages/Register.jsx`/`BusinessLocations.jsx` (place-detail fields + autocomplete race), `frontend/jsconfig.json` (types `["vite/client"]`), `frontend/.env.example` (new), `frontend/src/pages/Login.jsx`, `frontend/src/pages/SubscribeCheckout.jsx` (legal link rows)
- **Depends on:** `TASK-I1` (checkout model/routes/gateway), `TASK-G2` (reader snapshot), existing Phase C business registration routes
- **Spec:** `AGENTS.md` (monorepo conventions), `backend/services/payments.py` "prepared for Stripe integration", `frontend/src/api/appClient.js` checkout block, `frontend/src/pages/SubscribeCheckout.jsx` legacy card fieldset
- **Setup reference:** `backend/README.md` (migrations), `AGENTS.md`
- **Conventions:** `Follow AGENTS.md` (`/api/v1` prefix, camelCase JSON, no role middleware on open checkout routes, ASCII-only DB content, `.jsx` uses JSDoc casts, migration chained `6f8a0b1c2d3e -> b5c2d1f0a3e8`)
- **Definition of Done:** `Stripe: `services/payments.py` gains `client_secret` on `create_payment_intent` (real Stripe path), `retrieve_payment_intent` (Stripe read vs simulated store, same shape incl. `payment_method`), and `card_details_from_payment_method`; `CheckoutSession` gains `client_secret` column + to_dict `clientSecret` (migration `b5c2d1f0a3e8`); create persists it; confirm accepts a `paymentIntentId` (Stripe mode: requires `gateway.stripe_enabled`, verifies intent status `succeeded` + amount `int(round(session.amount*100))` match, resolves brand/last4 from the PaymentMethod, 400 otherwise), rejects raw card details when Stripe is enabled ("must be authorized through Stripe Elements"), and keeps PayPal/wallet simulated in Stripe mode (no finalize). Legacy raw-card + simulated gateway flows unchanged when no `STRIPE_SECRET_KEY`. Frontend: `toCheckoutSession` carries `clientSecret`; `SubscribeCheckout.jsx` renders a real Stripe `Elements`/`PaymentElement` fieldset when `VITE_STRIPE_PUBLISHABLE_KEY` is set (debounced session create on customer/plan/consent change, submit via `stripe.confirmPayment({redirect: "if_required", return_url})` then backend confirm with the confirmed `paymentIntentId`), with the legacy fieldset as an automatic fallback when the key is missing, the backend is offline/simulated, or create returns no clientSecret; `SubscribeSuccess.jsx` finalizes via `payment_intent` + `redirect_status=succeeded` query params before reading the session. Registration round-trip: business register payload already carries Geoapify place details (`geoapifyPlaceId`, `deliveryLatitude/Longitude`, `deliveryRegion`, `deliveryLocationName`) into the `CompanyAccount.lead.deliveryLocation` and the auto-created `BusinessLocation`; verified end to end (admin review sees license + location coordinates, approval unlocks business login, `GET /business/locations` returns the saved delivery site). Env: `STRIPE_SECRET_KEY` (backend) and `VITE_STRIPE_PUBLISHABLE_KEY` (frontend) placeholders in `.env.example`; `jsconfig.json` types now include `vite/client` for `import.meta.env`; clean checkout that old card-brand/server integration pre-dist. Doc-driven hardening (vs `https://docs.stripe.com/api` + stripe.js reference): PaymentIntent creation carries `metadata.sessionId` + `receipt_email` so the webhook can correlate and finalize an abandoned checkout; new `POST /api/v1/webhooks/stripe` verifies `STRIPE_ENDPOINT_SECRET` via `stripe.Webhook.construct_event` (400 on bad signature), applies `payment_intent.succeeded` (amount-checked, shared `_complete_checkout`/`finalize_from_stripe_intent` provisioner reused with the confirm route) and `payment_intent.canceled`, and acknowledges-no-ops unknown events; when no endpoint secret is configured only well-formed events are acknowledged unprocessed (safe sim mode). Frontend: `elements.submit()` runs before `confirmPayment`, `clientSecret` is passed explicitly, the debounced create skips when the payload is unchanged (no orphan PaymentIntents / Pay-button flicker), and `stripeLinkRef` is cleared whenever a new session is minted so stale Elements can never confirm into a newer session.`
- **Runtime Verification:** `PASSED. Business round-trip smoke `%TEMP%/opencode/business_registration_smoke.py` (11/11, Flask test_client + live MySQL): register business 201 pendingApproval; login blocked pre-approval 403; admin list shows company with workEmail/status "License submitted"/licenseDocument data-URL + lead.deliveryLocation carrying placeId + lat/long; admin detail returns exactly 1 auto-created BusinessLocation with name/placeId/lat/long/region/status "Review"; approve-license -> "License approved"; business login 200; business locations list includes the saved delivery site. Stripe-mode smoke `%TEMP%/opencode/checkout_stripe_smoke.py` (19/19) against the app test client: simulated card flow (create 201 + quotes, GET, confirm `Visa **** 4242`, idempotent replay), simulated PayPal flow, stub-SDK Stripe-mode (client_secret from create, raw-card confirm rejected 400 "Elements", `paymentIntentId` verify + brand/last4, amount-mismatch 400, unsucceeded intent 400, paypal stays simulated), and signed-webhook handling (`webhook/shared` locked: `payment_intent.succeeded` finalizes an "abandoned" checkout to `succeeded` + active subscription via `metadata.sessionId` correlation, `payment_intent.canceled` marks it `cancelled`, tampered signature 400, unknown-session events acknowledged 200 no-op). Migration `b5c2d1f0a3e8` applied (`flask db upgrade b5c2d1f0a3e8`; pre-existing branch heads 9ab4c7d2e130 unaffected). Frontend gates: `npm run typecheck`, `npm run lint`, `npm run build` (1804 modules) all clean after the Stripe.js hardening.`
- **Blockers:** `None`
- **Description:** `Wire real Stripe payments into the public checkout using Stripe.js: the gateway returns the PaymentIntent client_secret, the session row persists it (new migration), confirm verifies a Stripe-confirmed PaymentIntent instead of raw card data when Stripe is enabled while the simulated gateway keeps working without keys, and the checkout page renders a Stripe `PaymentElement` (with an automatic fallback to the legacy fieldset when Stripe is unavailable), plus the success page finalizes SCA redirect returns. The follow-up review pass aligned the integration with the Stripe docs: a signature-verified webhook finalizes checkouts whose browser never returned from 3DS, intents are linked to sessions via metadata, and the client-side confirm flow follows the documented `elements.submit()` + `redirect:"if_required"` pattern without minting throwaway intents on every keystroke. In the same pass, verify the business registration form round-trip: the delivery location (name, placeId, coordinates) is saved server-side, visible to the admin reviewer, and usable for delivery purposes after approval.`
